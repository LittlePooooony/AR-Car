...
namespace PathCreation {
    ...
    public class BezierPath {
        ...
        private List<int> dockedNum = new List<int>();

        public void addDockedList(int value)
        {
            dockedNum.Add(value);
        }
        public void removeDockedList(int value)
        {
            dockedNum.Remove(value);
        }
 ...
 centre + Vector3.right * width
        ...
        void AutoSetAnchorControlPoints(int anchorIndex) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (dockedNum.Contains(controlIndex)) continue;
        ...
        void AutoSetStartAndEndControls () {
            if (isClosed) {
                ...
                if (NumAnchorPoints == 2) {
                    ...
                    if (!dockedNum.Contains(1)) points[1] = (points[0] + points[2]) * .5f;
                    if (!dockedNum.Contains(points.Count - 2)) points[points.Count - 2] = (points[points.Count - 1] + points[points.Count - 3]) * .5f;
                }
            }
        }
